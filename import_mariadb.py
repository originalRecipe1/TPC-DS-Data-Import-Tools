#!/usr/bin/env python3
"""
import_mariadb.py - Import TPC-DS chunked data into MariaDB in parallel

This script imports TPC-DS data chunks into MariaDB using parallel connections.
Each chunk is imported concurrently to maximize performance.

Usage:
    python import_mariadb.py --data-dir /path/to/chunks --chunks 4
    python import_mariadb.py --combined-data /path/to/combined/tables
...
class MariaDBImporter:
    ...
    def import_chunk_file(self, table_name: str, container_path: str, chunk_id: Optional[str] = None) -> bool:
        """Import a single chunk file into MariaDB using container path"""
        ...
        try:
            with self.get_connection() as conn:
                ...
                # Use LOAD DATA INFILE with container path (not LOCAL INFILE)
                load_sql = f"""
                LOAD DATA INFILE %s 
...
def main():
    parser = argparse.ArgumentParser(description="Import TPC-DS data into MariaDB")
