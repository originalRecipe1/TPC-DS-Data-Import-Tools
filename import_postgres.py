#!/usr/bin/env python3
"""
import_postgres.py - Import TPC-DS chunked data into PostgreSQL in parallel

This script imports TPC-DS data chunks into PostgreSQL using parallel connections.
Each chunk is imported concurrently to maximize performance.

Usage:
    python import_postgres.py --data-dir /path/to/chunks --chunks 4
    python import_postgres.py --combined-data /path/to/combined/tables
...
class PostgresImporter:
    ...
    def import_combined_data(self, max_workers: int = 4) -> None:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks
            future_to_task = {
                executor.submit(self.import_chunk_file, table_name, file_path, chunk_id): 
                (table_name, chunk_id)
                for table_name, file_path, chunk_id in tasks
            }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Import TPC-DS data into PostgreSQL")
